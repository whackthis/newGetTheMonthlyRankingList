...
OUTPUT_CSV = "books_info-10.csv"
SLEEP_SECONDS = random.uniform(53.0, 56.0)
...
def parse_product_page(html: str, url: str) -> List[str]:
    soup = BeautifulSoup(html, "html.parser")
    ...
    for li in soup.select("li"):
        ...
        label, value = extract_label_value(text)

        if label == "作者" and not author:
            a_tags = li.select("a")
            if a_tags:
                names = [normalize_space(a.get_text(" ", strip=True)) for a in a_tags]
...
def main() -> None:
    ...
    # 先暖機，降低 403 機率
    try:
        session.get("https://www.books.com.tw/", timeout=20)
    except Exception:
        pass
