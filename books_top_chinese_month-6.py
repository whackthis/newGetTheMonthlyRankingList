...
class BooksScraper:
    ...
    def fetch_list_html(self) -> Tuple[str, str]:
        ...
        if LOCAL_LIST_FALLBACK.exists():
            text = LOCAL_LIST_FALLBACK.read_text(encoding="utf-8", errors="replace")
            return text, f"local fallback: {LOCAL_LIST_FALLBACK}"
            ...
            if key_lower == "isbn" and isinstance(value, str) and not result["isbn"]:
                ...
                for item in value:
                    if isinstance(item, dict) and isinstance(item.get("name"), str):
                        authors.append(normalize_space(item["name"]))
                if authors:
                ...
                match = DATE_RE.search(value.replace("-", "/"))
                result["publish_date"] = match.group(1) if match else normalize_space(value)
...
def extract_book_info_from_product_html(html: str) -> Dict[str, str]:
    ...
    if not result["isbn"]:
        ...
        if meta_description and meta_description.get("content"):
            match = ISBN_RE.search(meta_description["content"])
            if match:
                result["isbn"] = match.group(0)
...
def main() -> None:
    ...
    for idx, (rank_title, url) in enumerate(items, 1):
        ...
        print(" | ".join(row))
        rows.append(row)
        time.sleep(random.uniform(73.0, 76.0))
