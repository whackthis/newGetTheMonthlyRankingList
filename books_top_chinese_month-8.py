...
def normalize_has_ebook(text: str) -> str:
    text = normalize_space(text)
    if text in ("有電子書", "有"):
        return "有電子書"
    if text in ("無電子書", "無"):
...
def info_score(info: Dict[str, str]) -> Tuple[int, int]:
    populated = sum(
        1
        for key in FIELDS
        if key not in ("url", "has_ebook") and normalize_space(info.get(key, ""))
    )
...
def merge_info(*infos: Dict[str, str]) -> Dict[str, str]:
    ...
    for info in infos:
        ...
        for field in FIELDS:
            ...
            if field == "has_ebook" and value:
                value = normalize_has_ebook(value)
...
def detect_ebook_from_html(soup: BeautifulSoup) -> str:
    # 精準判斷：找 <span>電子書</span>
    for span in soup.find_all("span"):
        if normalize_space(span.get_text()) == "電子書":
            return "有電子書"

    # 備援：若 HTML 文字中有明確關鍵字，也視為有電子書
...
def extract_book_info_from_product_html(html: str, url: str) -> Dict[str, str]:
    ...
    result["has_ebook"] = detect_ebook_from_html(soup)
        ...
        try:
            with path.open('r', encoding='utf-8-sig', newline='') as fh:
                ...
                for row in reader:
                    ...
                    info['has_ebook'] = normalize_has_ebook(info.get('has_ebook', ''))
...
def fetch_book_info(
    ...
    *,
    rank_title: str,
    url: str,
    history_by_url: Dict[str, Dict[str, str]],
    history_by_title: Dict[str, Dict[str, str]]
...
def write_csv(rows: Iterable[List[str]]) -> None:
    with open(OUTPUT_CSV, 'w', newline='', encoding='utf-8-sig') as fh:
        ...
        writer.writerow(['書名', '作者', '原文作者', '譯者', '繪者', '出版社', '出版日期', 'ISBN', '是否有電子書', 'URL'])
...
def main() -> None:
    ...
    for idx, (rank_title, url) in enumerate(items, 1):
        ...
        print(f'  {info["has_ebook"] or "無電子書"}')
        ...
        row = [
            ...
            info['publish_date'],
            info['isbn'],
            info['has_ebook'] or '無電子書',
            url,
        ]
        print(' | '.join(row))
        rows.append(row)
        time.sleep(random.uniform(63.0, 66.0))
